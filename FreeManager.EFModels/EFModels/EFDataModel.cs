...
public partial class EFDataModel : DbContext
{
    ...
    // => optionsBuilder.UseSqlite("Data Source=C:\\Working\\FreeManager.db");
    // => optionsBuilder.UseMySQL("Server=localhost;Database=FreeManager;User=admin;Password=admin");
    // => optionsBuilder.UseNpgsql("Host=localhost;Database=FreeManager;Username=postgres;Password=admin");
    // => optionsBuilder.UseSqlServer("Data Source=(local);Initial Catalog=FreeManager;Persist Security Info=True;User ID=sa;Password=saPassword;MultipleActiveResultSets=True;TrustServerCertificate=True;");
